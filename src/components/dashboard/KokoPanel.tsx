...
export const KokoPanel = () => {
  ...
  useEffect(() => {
    const refresh = () => setCareerTitle(getChosenCareer()?.title || null);
    refresh();
    window.addEventListener("worthscope:progress", refresh);
    return () => window.removeEventListener("worthscope:progress", refresh);
  }, []);

  const message = careerTitle
    ? `You're working toward becoming a ${careerTitle}. Complete your first mission to unlock Phase 2.`
